 ...
 $.fn.extend({
     tabControl: function(options, tags) {
         ...
         var _tags = [];
         if (tags) {
             //将非中英文、数字、下划线、逗号的其他字符都去掉，且不能以逗号开头与结束
                 ...
                 $("input[type='text'][name='tabinput']:last", obj).bind("keydown blur click",
                     function(event) {
                         ...
                         if (event.keyCode == 13 || event.keyCode == 9 || event.type == "blur") {
                             //主要是為了tab鍵，不要讓當前元素失去焦點（即阻止（浏览器）默认的行为）
                             event.preventDefault();
                             event.stopPropagation();
                             var inputObj = $(this);
                             var value = $(this).val().replace(/\s+/gi, "");
                             if ((event.keyCode == 13 || event.keyCode == 9) && value != "") //主要是处理IE
                             {
                             ...
                             if (value != "") {
                                 ...
                                 {
                                    showMes($(this), "开始不能为特殊符号！");
                                    return;  
                                 }
                                 // 如果两个特殊符号出现在一起，报错
                                 if (value.match(/\『\『/) || value.match(/\『\；/) || value.match(/\『\』/) || value.match(/\；\『/) || value.match(/\；\』/) || value.match(/\；\；/) || value.match(/\』\『/) || value.match(/\』\；/))
                                 {
                                    showMes($(this), "两个特殊符号不能出现在一起！");
                                    return;
                                 }
                                 // 如果括号不匹配，报错
                                 var brackets = [];
         ...
         var compTab = function(obj, inputObj, value) {
             ...
             value = value.replace(/\；/g, '<font color=red>；</font>');
             value = value.replace(/\『/g, '<font color=green>『</font>');
             value = value.replace(/\』/g, '<font color=green>』</font>');
